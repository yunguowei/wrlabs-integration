...
//	printf("Device path is %s \n", dev_path);
	...
	//		printf("type %d\tcode %d\tvalue %f\n", ie.type, ie.code, ie.value);
			switch (ie.code){
				case 24:  //Barometer
					humidity = (float)ie.value/4096.0;
			 		break;
			 	case 9:   //Temperature
			 		temperature = (float)ie.value/100.0;
			 		break;
			 	default:
			 		break;
			}
		...
		if (humidity != OUT_OF_RANGE && temperature != OUT_OF_RANGE) {
		 	printf("Barometer = %5.2f hPa, Temperature = %5.2f C or %5.2f F\n", humidity, temperature, temperature * 1.8 + 32);
